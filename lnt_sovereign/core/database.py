...
class DecisionAudit(Base): # type: ignore
    """
    Standard audit table for decision tracking.
    Includes logic proof signature chaining.
    """
    ...
    logic_proof = Column(String(64))  # SHA-256 Hash
...
def get_last_hash(db: Session) -> str:
    ...
    if last_entry and hasattr(last_entry, 'logic_proof'):
        res: str = str(last_entry.logic_proof)
        return res
